...
void DC_motor::put(const uint8_t num, const int16_t duty,
                   const uint16_t duty_max) {
    // 想定外の入力が来たら何もしない
    if (duty_max > DUTY_SPI_MAX)
        return;
    if (abs(duty) > duty_max)
        return;
    if (num >= DC_MOTOR_NUM + SOL_SUB_NUM)
        return;
...
void DC_motor::send(void) {
    ...
    if (sign_buf == 0xFF) {
        for (int i = 0; i < (DC_MOTOR_NUM + SOL_SUB_NUM) * DC_MOTOR_BYTES;
             i++) {
            // digitalWriteFast(Pin(SS_DC_MOTOR_MISO),LOW);
            digitalWriteFast(Pin(SS_DC_MOTOR), LOW);
...
void Solenoid::put(const uint8_t num, const bool state) {
    if (num >= SOL_SUB_NUM)
        return;
    if (DC_motor::buf[DC_MOTOR_NUM + num] ==
        (state ? DUTY_SPI_MAX + 1 : -(DUTY_SPI_MAX + 1)))
        return;

    unsigned long time_now = millis();
    if (time_now - time_prev[num] < SOL_TIME_MIN)
        return;

    DC_motor::buf[DC_MOTOR_NUM + num] =
        (state ? DUTY_SPI_MAX + 1 : -(DUTY_SPI_MAX + 1));
...
bool Abs_enc::parity_check(const uint16_t enc_val) {
    ...
    if (bit[15] ==
            !(bit[13] ^ bit[11] ^ bit[9] ^ bit[7] ^ bit[5] ^ bit[3] ^ bit[1]) &&
        bit[14] ==
            !(bit[12] ^ bit[10] ^ bit[8] ^ bit[6] ^ bit[4] ^ bit[2] ^ bit[0])) {
        return true;
    }
    return false;
}
...
void Adc::receive(void) {
    ...
    for (int i = 0; i < DC_MOTOR_NUM; i++) {
        ...
        float raw_val =
            (float)(data - CURRENT_RES) / CURRENT_RES * CURRENT_MAX + bias[i];
...
void Cubic::update(const unsigned int us) {
    ...
    if (time_now < time_prev)
        dt = time_now + MICROS_MAX - time_prev;
    else
        dt = time_now - time_prev;
    time_prev = time_now;

    if (us > dt)
        delayMicroseconds((us - dt) * 2); // なぜか2倍すると正しい周期になる
